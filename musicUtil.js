...
if (targetCookie) {
  ...
  }).catch(err => console.error('[MUSIC] Failed to set YouTube cookie in play-dl:', err));
}

function parseCookieString(cookieStr) {
  if (!cookieStr) return [];
  let cleanStr = cookieStr.trim();
...
async function playSong(guildId, song) {
  ...
  try {
    console.log(`[MUSIC] Requesting audio stream using play-dl for song: "${song.title}" (${song.url})`);
    
    const playStream = await play.stream(song.url);
    const stream = playStream.stream;
